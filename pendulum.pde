class pendulum {
  ...
  void stepSingle(float stepSize) {
    ...
    // A bit of maths to try to get the display to work with different sized windows
    float scaleDist = 0.9*min(width, height)/2;
    x1 = width/2 + map(x1, -l1-l2, l1+l2, -scaleDist, scaleDist);
    x2 = width/2 + map(x2, -l1-l2, l1+l2, -scaleDist, scaleDist);
    y1 = height/2 + map(y1, -l1-l2, l1+l2, -scaleDist, scaleDist);
    y2 = height/2 + map(y2, -l1-l2, l1+l2, -scaleDist, scaleDist);
  }
